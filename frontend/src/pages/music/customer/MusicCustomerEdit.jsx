...
const CustomerEdit = () => {
  ...
  const { abcNotation } = location.state || {};
  ...
  const [abcContent, setAbcContent] = useState( abcNotation || "");
  ...
  useEffect(() => {
    if (!abcNotation) {
      // Redirect the user back to the compose page if no abcNotation is found
      navigate("/customer-compose/customer-new-compose", { replace: true });
    }
  }, [abcNotation, navigate]);
  ...
  //       // Render the ABC notation as sheet music
    ...
    if (splitContent.length > 0) {
      ...
      ABCJS.renderAbc("abc-render-edit", currentPageContent, options, {
        clickListener: (abcElem) => {
          if (abcElem && sheetMusicRef.current) {
            const { startChar, endChar } = abcElem;
            const textarea = sheetMusicRef.current;
  ...
  const handleSave = () => {
    ...
    setDialogMessage("Changes saved locally");
    setOpenDialog(true);
    setIsSaving(false);
  };
  ...
  const handleProceed = () => {
    navigate(
      "/customer-compose/customer-new-compose/customer-edit/customer-catalog",
      { state: { abcContent } }
    );
  };
