      ...
      const fetchSearchedArtworks = async () => {
        try {
          ...
          const filteredResults = response.data.filter((artwork) => {
            ...
            const hasMatchingProperty = Object.entries(artwork).some(
              ([key, value]) => {
                ...
                if (
                  ["_id", "imageUrl", "createdAt", "updatedAt", "__v"].includes(
                    key
                  )
                )
                  return false;
  ...
  const applyFilters = () => {
    ...
    if (hasFilter) {
      ...
      try {
        ...
        const artworksToFilter = searchQuery
        ...
        >
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
            ...
            {Object.keys(filterFields).length === 0 && (
              ...
              >
                No filter fields available. Please configure dynamic fields in
                the field management section.
              </Typography>
            )}
          </Box>
