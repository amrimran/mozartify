...
import { Box, Typography, Button, Card, CardContent, List, ListItem, ListItemText, Avatar, Divider, TextField , Select,
  MenuItem, FormControl, InputLabel, Switch, FormControlLabel, Pagination } from "@mui/material";
...
return () => {
  ...
  const transposeAbc = useCallback((abc, semitones) => {
    ...
    const renderAbc = useCallback((abcNotation) => {
      ...
      const visual = abcjs.renderAbc("abc-container", abcNotation, renderOptions)[0];
      setVisualObj(visual);
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      ...
      <Box sx={{ flexGrow: 1, p: 3, pl: 31 }}>
        ...
        <Box sx={{ display: "flex", gap: 4 }}>
          {/* Music Score Preview */}
          <Card sx={{ flexGrow: 1, p: 3, bgcolor: "#F2F2F5", borderRadius: 2, maxWidth: '800px' }}>
  ...
  >
    ...
    <TextField
      ...
      onChange={(e) => {
        ...
        if (!isNaN(newValue) && newValue >= 40 && newValue <= 200) {
          setTempo(newValue);
        }
      }}
      size="small"
...
>
  ...
  <FormControl size="small" sx={{ minWidth: 150 }}>
    ...
    {/* ABC notation container with fixed height */}
          ...
          {/* Music Score Details */}
          {metadata ? (
            <Card sx={{ width: 200, p: 2, bgcolor: "#F2F2F5", borderRadius: 2, height: "auto", maxHeight: "850px", overflowY: "auto", flexGrow: 1 }}>
